...
class LateFusion(nn.Module):
    ...
    def forward(self, data):
        ...
        preds = {
            k: self.sigmod(self.fc(v)).squeeze(-1)
            for k, v in features.items()
            if k in self.final_input_parts
        }

        losses = {
            f"{k}_loss": self.loss(v, data["label"].float()) for k, v in preds.items()
        }
        metrics = {
            "preds": torch.stack(list(preds.values()), dim=-1).mean(-1),
            "target": data["label"],
        }
